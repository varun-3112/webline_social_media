...
public class PostRepositoryImpl implements PostRepository{
	...
	public List<Post> getPostsByUser(String filterFlag, PostFilter postFilter, Long userId) {
		...
		sb.append(" ORDER BY "+postFilter.getSortedBy()+" "+postFilter.getSortType()+" "+env.getProperty("PAGINATION_QUERY"));
		...
		return jdbcTemplate.query(sb.toString(), (rs,rowNum) -> {
			return new Post(rs.getLong("post_id"),rs.getString("post_title"),rs.getString("post_content"),rs.getTimestamp("created_at"),rs.getTimestamp("updated_at"),rs.getString("image_path"),rs.getLong("likes"),rs.getLong("dislikes"));
	...
	public List<UserDto> getPostLikedByUsers(Long postId) {
		...
		return jdbcTemplate.query(GET_POST_LIKED_BY_USERS, (rs,rowNum) -> {
			return new UserDto(rs.getString("user_name"),rs.getString("email"),rs.getString("role"),rs.getTimestamp("liked_disliked_at"));
		},postId);
	}
	...
	public void likeDislikePost(Long postId, Long userId, String operation, String queryType) {
		...
		if(operation.equals("like")) {
			if(queryType.equals("ADD")) {
				jdbcTemplate.update(env.getProperty("ADD_LIKE_DISLIKE_FOR_POST"), postId,userId,"Y","N",new Timestamp(new Date().getTime()));
			}else {
				jdbcTemplate.update(env.getProperty("UPDATE_LIKE_DISLIKE_FOR_POST"), "Y","N",new Timestamp(new Date().getTime()),postId,userId);
			}
		}else {
			if(queryType.equals("ADD")) {
				jdbcTemplate.update(env.getProperty("ADD_LIKE_DISLIKE_FOR_POST"), postId,userId,"N","Y",new Timestamp(new Date().getTime()));
			}else {
				jdbcTemplate.update(env.getProperty("UPDATE_LIKE_DISLIKE_FOR_POST"), "N","Y",new Timestamp(new Date().getTime()),postId,userId);
			}
		}
	}
	...
	public Post AddUpdatePost(Post post,Long postId) {
		if(postId==0) {
			//Add new post
			jdbcTemplate.update(env.getProperty("ADD_PRODUCT_FOR_USER"),post.getPostTitle(),post.getPostContent(),post.getUserId(),post.getImagePath(),post.getCreatedAt(),post.getUpdatedAt());
		}
		else {
			//Update post
			jdbcTemplate.update(env.getProperty("UPDATE_PRODUCT_FOR_USER"),post.getPostTitle(),post.getPostContent(),post.getUserId(),post.getImagePath(),post.getUpdatedAt(),postId);
		}
		return post;
	}
	...
	public Long checkPostForDuplicate(Post post) {
		Long postId=0L;
		try {
			postId=jdbcTemplate.queryForObject(env.getProperty("CHECK_POST_FOR_DUPLICATE"), Long.class, post.getUserId(),post.getPostTitle());
		}
		catch(Exception e) {
			e.printStackTrace();
		}
